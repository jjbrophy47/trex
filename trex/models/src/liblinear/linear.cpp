/*
   ...
   Modified 2021:
   - Added alpha coefficients to model struct so they can be accessed after
     solve any problems involving dual optimization. - Jonathan Brophy
...
static int solve_l2r_l1l2_svc(
	...
	double Cp, double Cn, int solver_type, int max_iter, double *alpha_coef)
{
	int l = prob->l;
	int w_size = prob->n;
	int i, s, iter = 0;
...
int solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn,
					   int max_iter, double *alpha_coef)
{
	int l = prob->l;
	int w_size = prob->n;
	int i, s, iter = 0;
...
static int train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn, BlasFunctions *blas_functions, double *alpha)
{
	double eps=param->eps;
	...
	switch(param->solver_type)
	{
		...
		case L2R_L2LOSS_SVC_DUAL:
			n_iter=solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L2LOSS_SVC_DUAL, max_iter, alpha);
			break;
		case L2R_L1LOSS_SVC_DUAL:
			n_iter=solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L1LOSS_SVC_DUAL, max_iter, alpha);
			break;
		case L1R_L2LOSS_SVC:
		{
			problem prob_col;
		...
		case L2R_LR_DUAL:
			n_iter=solve_l2r_lr_dual(prob, w, eps, Cp, Cn, max_iter, alpha);
			break;
		case L2R_L2LOSS_SVR:
		{
			double *C = new double[prob->l];
...
model* train(const problem *prob, const parameter *param, BlasFunctions *blas_functions)
{
	...
	if(check_regression_model(model_))
	{
		...
		model_->nr_class = 2;
		model_->label = NULL;
		model_->n_iter[0] = train_one(prob, param, &model_->w[0], 0, 0, blas_functions, model_->alpha);
	}
	else
	{
		int nr_class;
		...
		else
		{
			if(nr_class == 2)
			{
				...
				model_->n_iter[0]=train_one(&sub_prob, param, &model_->w[0], weighted_C[1], weighted_C[0], blas_functions, model_->alpha);

                for (int i = 0; i < model_->alpha_size; i++) {
                    model_->perm[i] = perm[i];
                }
			}
			else
			{
				...
				for(i=0;i<nr_class;i++)
				{
					...
					model_->n_iter[i]=train_one(&sub_prob, param, w, weighted_C[i], param->C, blas_functions, model_->alpha);
