...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn, double *alpha_coef)
{
	int l = prob->l;
	int w_size = prob->n;
	int i, s, iter = 0;
                      ...
                      double *alpha)
{
	...
	switch(param->solver_type)
	{
		...
		case L2R_L1LOSS_SVC_DUAL:
			...
			break;
		case L2R_LR_DUAL:
			solve_l2r_lr_dual(prob, w, eps, Cp, Cn, alpha);  // alpha values are in here
			break;
		default:
...
model* train(const problem *prob, const parameter *param)
{
	...
	if(check_regression_model(model_))
	{
		...
		model_->nr_class = 2;
		model_->label = NULL;
		train_one(prob, param, model_->w, 0, 0, model_->alpha);
	}
	else
	{
        // fprintf(stderr, "Classification model\n");
		...
		else
		{
			if(nr_class == 2)
			{
				...
				train_one(&sub_prob, param, model_->w, weighted_C[0], weighted_C[1], model_->alpha);
			...
			else
			{
				...
				for(i=0;i<nr_class;i++)
				{
					...
					train_one(&sub_prob, param, w, weighted_C[i], param->C, model_->alpha);
...
int save_model(const char *model_file_name, const struct model *model_)
{
    ...
    // unwind perm indices to original indices
    int *unwinder = Malloc(int, model_->alpha_size);
    for (int i = 0; i < model_->alpha_size; i++) {
        for (int j = 0; j < model_->alpha_size; j++) {
            if (model_->perm[j] == i) {
                unwinder[i] = j;
                break;
            }
    ...
    fprintf(fp, "alpha\n");
    for (i = 0; i < model_->alpha_size; i++) {
        fprintf(fp, "%.8f ", model_->alpha[unwinder[i]]);
    }
    fprintf(fp, "\n");
