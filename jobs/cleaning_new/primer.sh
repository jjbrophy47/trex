...
method_list=(
             ...
             'klr_og_tree_output_sim_abs'
             'klr_og_leaf_path_alpha'
             'klr_og_leaf_path_sim'
             'klr_og_leaf_path_sim_abs'
             'klr_og_leaf_output_alpha'
             'klr_og_leaf_output_sim'
             'klr_og_leaf_output_sim_abs'
             'klr_og_weighted_leaf_path_alpha'
             'klr_og_weighted_leaf_path_sim'
             'klr_og_weighted_leaf_path_sim_abs'
             'klr_tree_output_alpha'
             'klr_tree_output_sim'
             'klr_tree_output_sim_abs'
             'klr_leaf_path_alpha'
             'klr_leaf_path_sim'
             'klr_leaf_path_sim_abs'
             'klr_leaf_output_alpha'
             'klr_leaf_output_sim'
             'klr_leaf_output_sim_abs'
             'klr_weighted_leaf_path_alpha'
             'klr_weighted_leaf_path_sim'
             'klr_weighted_leaf_path_sim_abs'
             'random'
             'fast_leaf_influence'
             'tree_loss'
)
...
for method in ${method_list[@]}; do
    for flip_frac in ${flip_frac_list[@]}; do
        job_name="CN_${dataset}_${model}_${method}_${flip_frac}"
        sbatch --mem=${mem}G \
               --time=$time \
               --partition=$partition \
               --job-name=$job_name \
