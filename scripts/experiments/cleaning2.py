...
def flip_labels(arr, k=100, rng=None, seed=1, indices=None, logger=None):
    """
...
def fix_noisy_instances(train_indices, noisy_indices,
    ...
    # get instances that are identified as noisy
    indices_to_fix = np.intersect1d(train_indices, noisy_indices)
    ...
    start = time.time()
    model, params = exp_util.train_tree_ensemble(clf, X_train, y_train_semi_noisy, param_grid,
                                                 scoring='accuracy', seed=args.rs)
    ...
    result['acc'] = acc
    result['auc'] = auc
    result['fixed_pct'] = float(indices_to_fix.shape[0] / noisy_indices.shape[0]) * 100
    result['noisy_indices'] = semi_noisy_indices
    result['y_train_noisy'] = y_train_semi_noisy

    return result


# def record_fixes(train_indices, noisy_indices, n_checkpoint):
def fix_noisy_instances2(train_indices, noisy_indices, n_check, n_checkpoint,
...
def random_method(y_train_noisy, n_check, rng):
    """
    Selects train instances to check uniformly at random.
    """
    train_indices = rng.choice(y_train_noisy.shape[0], size=n_check, replace=False)
...
def experiment(args, logger, out_dir):
    """
    Cleaning Experiment:
      1) Flip a percentage of train labels.
      2) Train a tree-ensemble on this noisy dataset (including tuning).
      3) Prioritize train instances to be checked using various methods.
      4) Check a fraction of the training data and fix any mislabeled instances.
      5) Repeat steps 2-4 until a fixed fraction of training instances has been checked.
    """
    ...
    param_grid = {'n_estimators': [10, 25, 50, 100, 250], 'max_depth': [3, 5, 10]}

    # reduce search hyperparameter search space for the Amazon dataset
    ...
    start = time.time()
    model, params = exp_util.train_tree_ensemble(clf, X_train, y_train, param_grid,
                                                 scoring='accuracy', seed=args.rs)
    logger.info('\ntraining clean model...{:.3f}s, params: {}'.format(time.time() - start, params))

    start = time.time()
    model_noisy, params_noisy_ = exp_util.train_tree_ensemble(clf, X_train, y_train_noisy, param_grid,
                                                              scoring='accuracy', seed=args.rs)
    logger.info('training noisy model...{:.3f}s, params: {}'.format(time.time() - start, params))
    ...
    # no. instances to check for each checkpoint
    n_check_total = int(y_train.shape[0] * args.check_pct)
    n_check = int(n_check_total / args.n_checkpoints)\

    print(n_check_total, n_check)

    # initialize results
    results = {}
    results['accs'] = [acc_noisy]
    results['aucs'] = [auc_noisy]
    results['checked_pcts'] = [0]
    results['fixed_pcts'] = [0]

    # initialize string
    s = '[Checkpoint] checked: {:.1f}%, fixed: {:.1f}%, Acc.: {:.3f}, AUC: {:.3f}, time: {:.3f}s'

    # perform a set number of ordering, checking and fixing, training and evaluation steps
    for i in range(args.n_checkpoints):

        # order training instances to be checked
        train_indices = sort_train_instances(args, y_train_noisy, n_check, rng=rng)[:n_check]

        # fix checked instances and re-evaluate
        result = fix_noisy_instances(train_indices, noisy_indices,
                                     clf, param_grid, X_train, y_train, X_test, y_test,
                                     logger=logger)

        # add result
        checked_pct = (n_check * (i + 1) / X_train.shape[0]) * 100
        print(checked_pct)
        results['checked_pcts'].append(checked_pct)
        results['accs'].append(result['acc'])
        results['aucs'].append(result['auc'])
        results['fixed_pcts'].append(result['fixed_pct'] + results['fixed_pcts'][-1])

        # update noisy labels
        noisy_indices = result['noisy_indices']
        y_train_noisy = result['y_train_noisy']

        # display progress
        if logger:
            logger.info(s.format(results['checked_pcts'][-1], results['fixed_pcts'][-1],
                                 results['accs'][-1], results['aucs'][-1], time.time() - start))

    # save results
    results['max_rss'] = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    results['total_time'] = time.time() - begin
...
def sort_train_instances(args, y_train_noisy, n_check, rng):

    # random
    if args.method == 'random':
        train_indices = random_method(y_train_noisy, n_check, rng=rng)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_dir', type=str, default='output/cleaning2/', help='output directory.')
