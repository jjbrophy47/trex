...
def process_results(df):
    ...
    groups = ['dataset', 'model', 'setting', 'train_frac_to_remove', 'method']
    ...
    for tup, gf in tqdm(df.groupby(groups)):
        ...
        # mean_vals, sem_vals, n_drop = mean_and_sem(med_loss)
        # main_result['med_loss_mean'] = mean_vals
        # main_result['med_loss_sem'] = sem_vals
...
def create_csv(args, out_dir, logger):
    ...
    experiment_settings = list(product(*[args.dataset,
                                         args.model,
                                         args.setting,
                                         args.train_frac_to_remove,
                                         args.method,
                                         args.rs]))

    # organize results
    results = []
    for dataset, model, setting, frac, method, rs in tqdm(experiment_settings):

        # create result
        template = {'dataset': dataset,
                    'model': model,
                    'setting': setting,
                    'train_frac_to_remove': frac,
                    'method': method,
                    'rs': rs}
...
if __name__ == '__main__':
    ...
    # I/O settings
    parser.add_argument('--in_dir', type=str, default='output/impact_test_set/', help='input directory.')
    parser.add_argument('--out_dir', type=str, default='output/impact_test_set/csv/', help='output directory.')
    ...
    parser.add_argument('--rs', type=int, nargs='+', default=list(range(1, 51)), help='random state.')
